...
********************************************************************
    ...
    RCS:      uiwpfdriver,v 3.0 2017年4月5日
    FROM:   2017年3月16日
********************************************************************
...
For Irontpython
Usage:    uiwpfdriver.exe <base64_request>
'''
...
def mapcore(b64req):
    ...
    if action in element_actions:
        ...
        WPFElement.SetHandle(r_kwargs.get("CURRENT_HANDLE",0));# after building to exe for command line, we can use handle to remamber the window which the previous action want to switch to.
    ...
    response.update({"resp":{"result":result,"errmsg":errmsg,"globals":WPFElement.GetVar()}});# GetVar also wiil response the handle named CURRENT_HANDLE
    resp = crypt(response)
...
def main_server():
    ...
    while True:
        ...
        newsock,address=sock.accept();#阻塞，并等待链接
